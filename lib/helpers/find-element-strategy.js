    ...
    // any selector containing given text (or any element containing text)
        ...
        if (query[0]) {
            ...
            if (query[0].charAt(query[0].length - 1) === '*') {
                ...
                value = '(' + cssToXPath(query[0]).slice(1) + '//*[not(self::script) and not(self::style)][contains(., ' + xPathStringLiteral(query[1]) + ')])[last()]';
            // otherwise return the element which is specified in selector
            } else {
                value = cssToXPath(query[0]).slice(1) + '[contains(., ' + xPathStringLiteral(query[1]) + ')]';
            }
        // only text
        } else {
